...
class opt_path(object):
    ...
    def prePath(self):
        ...
        """
        for ini_state in self.pba_graph.graph['init']:
            for accept_state in self.pba_graph.graph['accept']:
                p = {(ini_state, accept_state) : A.aStar(self.pba_graph, ini_state, accept_state, 0)}
    ...
    def sufPath(self):
        ...
        for accept_state in self.pba_graph.graph['accept']:
            ...
            for next_state in self.pba_graph.succ[accept_state]:
                pre_cost = self.pba_graph.succ[accept_state][next_state]['cost']
                next_path = A.aStar(self.pba_graph, next_state, accept_state, pre_cost)
    ...
    def optRun(self):
        ...
        for (state, path) in iter(self.pre_path.items()):
            ...
            if not path == None and (state[1], state[1]) in self.suf_path.keys():
                p = {state: (path[0] + self.suf_path[(state[1], state[1])][0], (path[1], self.suf_path[(state[1], state[1])][1] ))}
    ...
    def printOptPath(self, ts_graph):
        ...
        for state in self.optimal_path[1][0]:
            optimal_pre.append(ts_graph.nodes[state[0]]['label'][0])
    
        optimal_suf = [optimal_pre[-1]]
        for state in self.optimal_path[1][1]:
            optimal_suf.append(ts_graph.nodes[state[0]]['label'][0])
